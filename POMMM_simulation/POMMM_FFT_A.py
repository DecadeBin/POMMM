...
a_rate = 2 # Aperture expended rate
L_full = L * a_rate
...
U_out1 = torch.fft.fftshift(torch.fft.fft(U_in, dim=0), dim=0)
U_out2 = torch.fft.fftshift(torch.fft.fft(U_out1 * weight, dim=1), dim=1)
del [U_out1]

U_out1_c = torch.fft.fftshift(torch.fft.fft(U_in_c, dim=0), dim=0)
U_out2_c = torch.fft.fftshift(torch.fft.fft(U_out1_c * weight_c, dim=1), dim=1)
del [U_out1_c]

# Compute intensity
I_out = torch.abs(U_out2)
I_out_a = I_out.numpy()

I_out_c = torch.abs(U_out2_c)
